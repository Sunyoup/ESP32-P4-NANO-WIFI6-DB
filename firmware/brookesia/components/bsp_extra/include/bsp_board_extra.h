...
#define CODEC_DEFAULT_VOLUME                (100)
...
/**
 * @brief Snapshot of the internal Ethernet interface state.
 */
typedef struct {
    bool initialized;
    bool started;
...
/**
 * @brief Initialize the board Ethernet interface.
 *
 * The interface is started without requiring a cable to be connected. Link and
 * IP state are updated asynchronously by the Ethernet and IP event handlers.
 */
esp_err_t bsp_extra_ethernet_init(void);

/**
 * @brief Get a consistent snapshot of the board Ethernet state.
 */
esp_err_t bsp_extra_ethernet_get_info(bsp_extra_ethernet_info_t *info);

/**
 * @brief Ping the current Ethernet gateway once.
 *
 * This function blocks until the request succeeds or times out and should be
 * called from a worker task rather than the LVGL task.
 */
